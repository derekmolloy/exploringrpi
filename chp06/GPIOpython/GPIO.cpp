...
int GPIO::setDirection(int dir){
   switch(dir){
   case 0: return this->write(this->path, "direction", "in");
      break;
   case 1:return this->write(this->path, "direction", "out");
      break;
   }
   return -1;
}
...
int GPIO::setValue(int value){
   switch(value){
   case 1: return this->write(this->path, "value", "1");
      break;
   case 0: return this->write(this->path, "value", "0");
      break;
   }
   return -1;
}
...
int GPIO::toggleOutput(){
	this->setDirection(1);
	if ((bool) this->getValue()) this->setValue(0);
...
void* threadedToggle(void *value){
	...
	while(gpio->threadRunning){
		if (isHigh)	gpio->setValue(1);
		else gpio->setValue(0);
...
int GPIO::waitForEdge(){
	this->setDirection(1); // must be an input pin to poll its value
