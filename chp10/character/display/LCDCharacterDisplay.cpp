/*
 * LCDCharacterDisplay.cpp  Created on: 24 May 2015
...
int LCDCharacterDisplay::setCursorPosition(int row, int column){
   ...
   for(int i=0; i<8; i+=2){
      offsets[i] = (i/2) * width;
      offsets[i+1] = offsets[i] + 0x40;
   }
//int offsets[8] = {0x00, 0x40, 0x10, 0x50, 0x20, 0x60, 0x30, 0x70};
   int value = (offsets[row] + column) | LCD_DDRAM_ADDR;
   this->command(value);
//   printf("[%02x]", value);
   return 0;
}
